---
title: "Predicting Alternative Fuel Usage with Random Forests"
...
```{r vessels, include= F}
setwd("C:/Users/Huawei/OneDrive - CBS - Copenhagen Business School/R Working Directory/MRV Projects/")
...
```
Now we can start with splitting the data into a training and testing data set. The random split is 75/25 between the training and testing set and stratified for our imbalanced outcome variable to ensure that the class probabilities stay roughly the same in the two resulting data sets.
```{r, message=F}
library(tidymodels)
...
```
Afterwards, we can see which candidate model performed best according to the J-statistic and collect the hyperparameter values from the best performing model.
```{r}
rf_tuned %>% show_best(metric = "j_index",n = 5)

rf_best_params <- rf_tuned %>% select_best("j_index")
```
The last step now is update the random forest model with the identified hyperparameter values and fit the model on last time on the complete training set and then evaluate the model performance with the unseen test set. 
